...
$(function() {
    ...
    function getPokemonInfo(NameOrID) {
     //We need a way to asynchronously handle making the API call and doing stuff when we get a respinse since we don't know how long it will take to get the response.

     //If we tried to write synchronous code (code that runs one line after another) this could cause problems if we try to pull information from the API respinse before we get it back.
